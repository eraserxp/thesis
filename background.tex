...
\oneline{
D^{\dagger}(R) T_{q}^{(k)} D(R) = \sum_{q^{\prime} =-k}^{k} D^{k\, *}_{q q^{\prime} }(R) T_{q^{\prime}}^{(k)} \ . \label{eqn:sphericalTensorDef1}
}
Replacing the rotation $R$ by its inverse $R^{-1}$, the above definition can be recasted into the following form:
\oneline{
D(R)  T_{q}^{(k)} D^{\dagger}(R)  = \sum_{q^{\prime} =-k}^{k} T_{q^{\prime}}^{(k)} D^{k}_{q^{\prime} q }(R) \ . \label{eqn:sphericalTensorDef2}
}
...
\oneline{
T^{(k)}_{q} = \left( \frac{4\pi k!}{(2 k + 1)!!} \right)^{1/2} Y_{k q} \ , \label{eqn:relationTwithY}
}
where 
\multiline{
n!! = \left\{ 
