...
Carr, Carr2, Trefzger2010, Kestner2011, gorshkov, gorshkov2} to the study of novel quasi-particles \cite{biexcitons} that cannot be realized in solid-state crystals.
 In the limit of strong trapping field,  each site of an optical lattice is populated by a fixed number of ultracold atoms or molecules. 
...
The paper has the following structure. Sections 2  and 3 present the results in terms of the general 
Hamiltonian parameters.  Section 4 addresses the particular case of ultracold atoms and molecules.
 Section 5 discusses controlled energy transfer in systems with, specifically, 
dipole - dipole interactions. Section 6 considers the effects of lattice vacancies on the possibility of controlling 
energy transfer and Section 7 extends the proposed technique to
control of excitation dynamics in strongly disordered arrays with a large
concentration of impurities. Section 8 presents the conclusions.
...
|\psi_{\rm exc}(k)\rangle$ represents a quasi-particle called
...
localization. Similar to how Eq.~(\ref{basis}) defines the collective excited states in the basis of lattice sites, 
any localized excitation $| \psi \rangle$ can be generally written as a coherent superposition of the exciton states
 $| \psi_{\rm exc}(k)\rangle$ with different $k$:

\begin{eqnarray}
| \psi \rangle = \sum_{k} G_k | \psi_{\rm exc}(k) \rangle.
\label{k-rep}
\end{eqnarray}
...
Here we consider the transformations leading to acceleration or deceleration of collective excitations, while   
 the focusing phase transformations are described in Section 3. 
...
in the expansion (\ref{basis}), so that each $| \psi_{\rm exc}(k)
\rangle$ component in a wave packet is transformed into $|
\psi_{\rm exc}(k+\delta)\rangle$. This transformation shifts the
...
shape evolution. The feasibility of such transformation in an ensemble of atoms or molecules on an optical lattice is discussed below and in Section 4. 
...
\caption{Example of controlled energy transfer in a one-dimensional array of quantum 
monomers subjected to a linear phase transformation. The graph illustrates the evolution of the exciton wave packet   centered at $k=0$ and initially positioned at
 the center of the array. The phase of the wave function
...
of 201 monomers with $\alpha = 22.83$ kHz and $\Delta E_{e-g} =
12.14$ GHz,  and the linear phase transformation $\Phi_n \simeq \Phi_0  -1.29 n$. 
As described in Section 4, these parameters correspond to
LiCs molecules trapped on an
optical lattice with lattice constant $a = 400$ nm and subjected
to a homogeneous DC field of 1 kV/cm directed perpendicular to the
intermolecular axis. The kicking potential leading to
  this particular  phase transformation  can be provided by a
$\lambda = 1064$ nm Gaussian laser beam, with the propagation
direction along the array axis, focused to 5 $\mu$m, with the
intensity at the focus equal to $10^7$ W/cm$^2$. The laser pulse
is on between 0 and  $3$ $\mu$s. 
} \label{momentum-kick}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
polar molecules trapped in an optical lattice, as described in detail in Section 4.
The time-dependent perturbation has the form of a short pulse with the duration $T = 3\; \mu s$. %$\tb{T} \approx \frac{1}{15} \hbar/\alpha$.
The phase acquired by the particles during this time is given by $\Phi_n \simeq \Phi_0  -1.29 n $, which can be achieved with a focused laser beam, as described in Section 4.
...
At weak parity-mixing fields considered here, the exciton-exciton interactions insignificantly mix different $k$ states
 ...
 the disorder-induced perturbations,  discussed in Sections 6 and 7.
...
of the coefficients $C_n(t)$ in Eq. (\ref{basis}). In order to find the expression for $C_n(t)$, we expand the amplitudes
...
$E(q)$ representing the exciton energy given by Eq.~(\ref{dispersion}).
...
As Eq. (\ref{Cn(t)}) shows, the focusing efficiency is determined by the phase transformation and
...
To answer this question, we assume that $C_{n = n_0}(t=0) = 1$ and apply Eq. (\ref{Cn(t)}) to
...
\end{equation}
For the time $t_*$ in Eq. (\ref{focus-time}) to be positive, $\alpha$ and $\Phi_0^*$ must have the same sign. Therefore, a convex quadratic phase profile
...
If $E(k)$ in Eq.~(\ref{Cn(t)}) is approximated as $E(k) = \D E_{e-g}
...
In order to derive Eq. (\ref{C_plane_wave}), we
...
in Eq.~(\ref{C_plane_wave}) are canceled, and the sum over $q$ reduces to a
...
According to Eq.~(\ref{C_plane_wave}), the dimensionless width
...
Using Eq.~(\ref{approx integral}) we find that for an arbitrary
...
Eqs. (\ref{x-focusing-Gauss}) and (\ref{x-focusing-PlaneWave}) are
...
explicitly discussed in Section 6. 
...
The techniques proposed in Sections 2 and 3 can be realized with ultracold atoms or molecules trapped in an
...
Once collective excited states are created, the phase-kicking technique introduced in Section 2, 
...
\mathcal{E}_{{\ast}}$, and using Eqs. (\ref{phase}) and
(\ref{DressedEnery-Sum}), gives  $\delta = 4 A
...
the center of the wave packet and the beam center, and ${\sigma}_x^{(2d)}$ is the width (in the coordinate representation) of the two-dimensional wave packet. Using Eqs. (\ref{phase}), (\ref{DressedEnery-Sum}) and (\ref{gaussian-intensity-profile}), we estimate the momentum kick by such a pulse as $
%\begin{equation}
...
given by the laser field pulse (\ref{intensity-varying}) with parameters $I_c$ and $I_1$ derived from Eq. (\ref{gaussian-intensity-profile}) with $z_0 = 45\; \mu$m and
...
Eq. (\ref{gaussian-intensity-profile}) can be approximated
...
profile which can be used to focus a wave packet  in a system with negative couplings $\alpha$ (see Section 3).
...
after a certain lattice site separation $n-m$ for the 1D system. The results become  converged (to within 0.2 \%) when
 ...
 couplings are in good agreement  with the analytical predictions given by Eqs. (\ref{focus-time}) and (\ref{x-focusing-Gauss}), the full calculations reveal that long-range 
...
to a desired region of the lattice by the phase transformations discussed in Section 3. For concreteness, we perform calculations for the system described in Section 4,
...
as described in Section 3, in order to focus the collective excitation at time $t_{\ast}$ to
...
determined by the disorder potential and the energy transfer becomes highly inefficient (however, see Section 7). On the other hand, vacancy
...
from different blocks can be made to interfere constructively by adding
 ...
 a sum of $M$ quasi-random phasors in Eq. (\ref{blocks-contribution}) \cite{opaque-1}.
...
in Eq. (\ref{planewave-initial}) are equal, so that  the phases $\phi_\gamma$ required for
block $\gamma$ are
\begin{equation}
