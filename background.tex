...
for $m\ge 0$ and $\Theta_{lm}(\theta) =(-1)^{m} \Theta_{l-m}(\theta) $ for $m<0$, and $P_l^{m}(\cos\theta)$ are Legendre functions. Sometimes, it is more convenient
...
&=&  \sum_{m^{\prime}}  \ket{j, m} D^{j}_{m^{\prime}, m}(R) \ .
}
...
\oneline{
\ket{(j_1 j_2)j, m} = \sum_{m_1, m_2} \ket{j_1, m_1; j_2, m_2} \braket{j_1, m_1; j_2, m_2} {(j_1 j_2)j, m} \ ,
}
