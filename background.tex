...
where $T^{(1)}_{s }$ is the tensor component in space-fixed axis system and the last equality comes from \autoref{eqn:relationDandY}. Substituting \autoref{eqn:dipoleSpaceFixed} into  \autoref{eqn:tensorComponents}, we
obtain:
\multiline{
...
\clearpage
For a molecule in site $n$, it must be in some state, therefore the occupation operators of all states sum up to the identity
operator:
\begin{equation}
...
$b_{nf}$ destroys a state $f$ in molecule $n$.  It follows from Eq. (\ref{single-molecule-operator}) that the 
...
excited state from the vacuum state $\ket{0}$ and $P_{nf}$ does the reverse.
...
\end{equation}
holds for any excited state $e$. This means that we can ignore the operators $\hat{N}_{ne}$ in practice. However, we cannot
...
In the first step, we express the Hamiltonian in terms of the $b$ operators. The result is
\multiline{
...
state $g$ and one excited state $e$. Then $f$, $f'$, $l$ and $l'$ in \autoref{eqn:hamInb} can only take $g$ or $e$. 
...
very similar but tedious. So I choose to use mathematica to write a symbolic program to handle the derivations. For 
reference, the result is given below. The Hamiltonian can be divided as follows:
\oneline{
H = H^{(0)} + H^{(1)} + H^{(2)} + H^{(3)} + H^{(4)}\ ,
}
where the superscripts ``$(n)$'' means that the corresponding term contains $n$ exciton operators. In the following, we 
...
$H^{(2)}$ can be further divided into the part that conserves particle numbers and the part that doesn't:
\multiline{
...
\begin{itemize}
\item the ground-state wavefunction of the crystal is the product of the ground-state wavefunction of individual molecule that constitute the crystal
% 
...
To understand the problem better, we analyze the Hamiltonian in \autoref{eqn:hamConserving} term by term. 
...
parity, the above matrix elements are zero. Under these circumstance, the dynamic interaction is zero and $H_4$ can 
...
dynamic interaction because its matrix elements in the one-excitation subspace vanish. For illustration purpose, we give the derivation here. Based on \autoref{eqn:excitonCommutationRelation}, it can be shown that the
...
The above analysis shows that the Hamiltonian can be simplifed to
\oneline{
...
The Hamiltonian in the Heitler-London approximation can be diagonalized analytically. Before showing the analytical way 
for diagonalization, we first introduce the concepts of site representation and quasimomentum representation. 
Previously,  the excitation creation and annihilation operators $P_{nf}$ and $P_{nf}^{\dagger}$ are defined with respect 
to the lattice site $n$. 
These operators form the site representation. Due to the periodicity of the crystal, we can use the quasimomentum representation, to represent excitations by the Fourier transforms of $P_{nf}$ and $P_{nf}^{\dagger}$: 
\begin{eqnarray}
