...
Several experiments have recently demonstrated that ultracold molecules can be trapped in the periodic potential of an
...
The $\hat{P}_n$ and $\hat{P}_m$ operators satisfy the Pauli commutation relation, as shown by   
\autoref{eqn:fermionCommutation} and \autoref{eqn:bosonCommutation}. More specifically, by omitting the subscript ``$e$'' in these two equations, we have the fermionic commutation
\begin{equation}
\p{n}\pd{m} + \pd{m}\p{n} = 1 \  ,
\end{equation}
...
As shown in \autoref{sec:eigenstateExcitonHam}, the Hamiltonian of  \autoref{exciton} can be diagonalized by the Fourier transforms: $\hat{P}^\dag(k) = \frac{1}{\sqrt{N_{\rm mol}}}
...
where 
\oneline{
\Tilde{D}(q) = \sum_n D(n) e^{-iqn} \ . \label{eqn:DTilde}
} 
...
\underbrace{ \frac{1}{2} \sum\limits_{n,m }^{N_{\rm mol}} D(n-m)\hat{P}_{n}^\dag \hat{P}_{m}^\dag \hat{P}_{n} \hat{P}_{m} }_\text{\circled{3}} \ , \label{eqn:biexcitonHamSplit}
}
...
The dimension of the system of coupled equations (\ref{eqn:forAllnm}) is about $N_{\rm mol}\times N_{\rm mol}$, which is large even for a lattice
 ...
 symmetry of the lattices and convert \autoref{eqn:forAllnm} into the quasimomentum representation. Since the
 conversion involves some nontrivial derivations, we give the details for \text{\circled{8}} in \autoref{eqn:forAllnm}
 which are characteristic for the whole calculations, namely
\multiline{
...
With all these results, \autoref{eqn:forAllnm} reduces to an equation for $C_{k_1, k_2}$, namely
\multiline{
...
There is an additional constraint for $C_{k_1, k_2}$ which is due to the assumption that $C_{n, n} = 0$, that is
\multiline{
...
&=& \sum_{n} C_{n,n} e^{i K n} = 0  \ . \label{eqn:cnnConstraint}
}
Because of \autoref{eqn:cnnConstraint}, we can eliminate $E$ terms in the first summation on the right hand side of
...
Note in the above equation, the range of the wavectors $k_1$, $k_2$ and $k_3$ is $[-\pi, \pi)$ so that the range of 
...
To begin, let's examine the coefficients $C_{n,m}$ in the most general two-exciton wavefunction of 
...
Substituting  \autoref{eqn:CKk} and \autoref{eqn:GKConvertedToGn} into \autoref{eqn:eqToStart} gives for each term:
\oneline{
\text{\circled{10}} = \frac{1}{\sqrt{N_{\rm mol}}} \sum_{l} C^{K}(l) e^{- i (k_1 - k_2 ) l /2} \ ,
}
\multiline{
\text{\circled{11}} &=& \frac{1}{N_{\rm mol}} \sum_{m} G^{K}(m) e^{- i (k_1 - k_2 ) m /2} \sum_{k_1^{'} + k_2^{'} = k_1 + k_2 } \sum_{n} D(n) e^{i (k_1 - k_1^{'} )n}\nonumber \\
&&  \hspace{2cm} \times \frac{1}{\sqrt{N_{\rm mol}}} \sum_{l} C^{K}(l) e^{- i (k_1^{'} - k_2^{'} ) l /2} \nonumber \\
\nonumber \\
...
\nonumber \\
&=&  \frac{1}{\left(N_{\rm mol}\right)^{3/2}  }\sum_{m,n,l} G^{K}(m) e^{- i (k_1 - k_2 ) m /2} D(n) e^{-i k_1 n }   C^{K}(l)   e^{i (k_1 + k_2 ) l /2}  \sum_{\substack{k_1^{'} + k_2^{'} \\= k_1 + k_2} } e^{i k_1^{'} (n-l)}\nonumber \\
...
\multiline{
\text{\circled{12}} &=& \frac{1}{N_{\rm mol}} \sum_{m} G^{K}(m) e^{- i (k_1 - k_2 ) m /2} \sum_{k_1^{'} + k_2^{'} = k_1 + k_2 } \left[ E_0 + \sum_{n} J(n) e^{i k_1^{'} )n} \right] \nonumber \\
&&  \hspace{2cm} \times\frac{1}{\sqrt{N_{\rm mol}}} \sum_{l} C^{K}(l) e^{- i (k_1^{'} - k_2^{'} ) l /2} \nonumber \\
\nonumber \\
&=&  \frac{1}{\sqrt{N_{\rm mol}} }\sum_{m,n,l} G^{K}(m) e^{- i (k_1 - k_2 ) m /2} e^{ i (k_1 + k_2 ) l /2} \nonumber \\
&& \hspace{0.2cm}\times\left[
E_0   C^{K}(l) \frac{1}{N_{\rm mol}} \sum_{k_1^{'} + k_2^{'} = k_1 + k_2 } e^{-i k_1^{'} l} + J(l) C^{K}(l)  \frac{1}{N_{\rm mol}} \sum_{k_1^{'} + k_2^{'} = k_1 + k_2 } e^{i k_1^{'} (n-l)} \right] \nonumber \\
\nonumber \\
...
&=& \frac{1}{ \sqrt{N_{\rm mol}} }\sum_{m,l} G^{K}(m) e^{- i (k_1 - k_2 ) m /2} J(l) C^{K}(l) e^{ i K l/2} 
}
and 
\oneline{
\text{\circled{13}} = \frac{1}{\sqrt{N_{\rm mol}} }\sum_{m,l} G^{K}(m) e^{- i (k_1 - k_2 ) m /2} J(l) C^{K}(l) e^{- i K l/2} \ .
}
...
\oneline{
\text{\circled{11}} = \frac{1}{ \sqrt{N_{\rm mol}} } \sum_{m,l}G^{K}(l-m) D(m) C^{K}(m) e^{-i (k_1 - k_2 ) l/2} \ ,
}
so that  \autoref{eqn:eqToStart} reduces to
\oneline{
C^{K}(l) = \sum_{m} C^{K}(m)\left[ G^{K}(l-m)D(m) - 2 G^{K}(l) J(m)\cos(K m/2) \right] \ .
}
...
\end{eqnarray}
where $r=n-m$ is the distance between two excitations. Since $J_{K} = 2 J \cos(K/2) < D$, we can see from the 
...
where $D$ and $J$ are the coupling constants $D(n-m)$ and $J(n-m)$ for $n-m =1$. In 3D crystals, biexcitons can form
 ...
 (NNA) is 
\oneline{
E_b(K) = 2E_0 + D + \frac{4 J^2 \cos^2(aK/2)}{D} \ ,
} 
...
For LiCs molecules, the condition of \autoref{biexc-formation} is satisfied for electric fields $>3.6$~kV/cm. Note that the
 ...
 sign of the  effective mass \cite{felipe}: negative $J$ corresponds to positive $m_{\rm eff}$ and vice versa, as seen
 in \autoref{fig:HowDJChanges} (d). Due to the linearity of the Schr\"{o}dinger equation, a positive potential is
 ...
 Because the signs of $J$ and $D$ are the same (and consequently the signs of $D$ and $m_{\rm eff}$ are opposite), the
 dynamical interaction of \autoref{dynamical} between excitons in this system is attractive.
...
where $K = k_1+k_2$ and $k = (k_1 - k_2)/2$, and $k_1$ and $k_2$ denote the wavevectors of the interacting
 ...
 \Autoref{fig:biexcitonSpectrum} (c) shows that for $\theta = 90^o > \theta^*=\arccos(1/\sqrt{3})$ the biexciton energy is
...
system is 
\oneline{
\hat{H}_{g-e-f} = \hat{H}_{\rm exc} + \hat{H}_{\rm dyn} +  \hat{H}_2 + \hat{H}_{12}  \ ,
}
 where
\oneline{
\hat{H}_2 = E_f \sum\limits_n \hat{R}_n^\dag \hat{R}_n   + \sum\limits_{n,m \neq n} J_{g-f}(n-m) \hat{R}_n^\dag \hat{R}_m
}
 and 
\oneline{
J_{g-f}(n-m) = \langle g_{n},f_{m} | V_{dd}(n-m) | f_{n},g_{m} \rangle \ .
}
...
Similar to the case for biexcitons, we define a three-exciton wavefunction in the site representation as
\oneline{
...
Since two excitations can't exist at the same molecule, we have the constraint on $C_{n_1, n_2, n_3}$ that
\oneline{
C_{n_1, n_2, n_3} = 0 \;\; \mbox{\rm if $n_1 = n_2$ or $n_2 = n_3$ or $n_1 = n_3$} \ . \label{eqn:trimerConstraint}
} 
After  derivations similar to those used  for the biexciton case, \autoref{eqn:schrodingerTrimer} and
...
Note that this equation can't be used to determine the values of $C_{n_1, n_2, n_3}$ when any two of $n_1$, $n_2$, and $n_3$ are equal. Using the Fourier transform
\oneline{
...
when any two of $n_1, n_2, n_3$ are equal, we need to take care of this constraint when solving
...
%using the method proposed in Ref. \cite{demille}. 
