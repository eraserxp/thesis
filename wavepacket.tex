...
In this subsection, we show some numerical details for the calculations presented in 
\autoref{sec:anisotropy}.

As discussed in \autoref{sec::ddInteraction}, the dipole-dipole interaction between molecule $A$ and $B$ separated by 
...
physical properties of the molecular array as the relative orientation of the the DC field with respect to the
 intermolecular axis remains the same. Therefore for the 1D case we have the freedom to choose $\phi_{R} = 0$. 
Now, let's turn to the 2D case. Since the physical properties of the system is only dependent on the angle 
$\theta_{R}$, we can decompose the 2D molecular array into many pairs of molecules with different intermolecular 
axes and  then treat them in the same way as the 1D case. 
