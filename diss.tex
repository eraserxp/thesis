...
%%   balanced
%%       when double-sided, ensure page content is centered
...
%This packages makes the hyperref package and the natbib package with options
%"numbers" and "sort&compress" work together. This means, that the citations
%(e.g [3,2,1]) will be compressed to [1-3], where the "1" and the "3" are
%(color-)linked to the bibliography. If you don't use it, the pages numbers that
%are supposed to appear at the end will not appear
\usepackage{hypernat}

% draw arrows with text over it
...
%command to produce a column vector
% usage \colvec{5}{a}{b}{c}{d}{e}.
\newcount\colveccount
\newcommand*\colvec[1]{
...
%command to produce a two by two matrix
% usage \mat{1&2 \\ 3&4} gives 
% ( 1   2 )
% ( 3   4 )
\newcommand{\mat}[2][ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc]{\left(
                                                \begin{array}{#1}
                                                    #2\\
                                                \end{array}
                                                \right) }

% usage \matrixTwoByTwo{1}{2}{3}{4}
%\newcommand{\twoByTwo}[4]{%
%\left(
%  \begin{array}{cc} 
%     #1 & #2 \\ 
...
%command to produce a three by three matrix
% usage \matrixThreeByThree{1}{2}{3}{4}{5}{6}{7}{8}{9}
...
\title{Quantum control of dynamics of quasiparticles in periodic and disordered lattice potentials}
...
%% files
\hypersetup{
  pdftitle={PhD thesis of Ping Xiang (DRAFT: \today)},
...
% allow long equations to appear on two pages
%page breaks are allowed, but avoided if possible
