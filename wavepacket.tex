...
The derivative of exciton dispesion curve $E(k)$ with respect to the wavevector $k$ determines the propagating 
behavior of an exciton wavepacket in real space. This can be easily shown by the simple derivations. 
...
&=&\frac{1}{\sqrt{N}} \sum_{n} e^{i \left[k n - \frac{E(k)}{\hbar}t \right]} \ket{n(t=0)} \label{eqn:kState}
}
...
state $\ket{k}$. So the plane wave $\ket{k}$ doesn't move in real space, which is expected. In the case of a
 wavepacket which has a Gaussian distribution in real space, the time evolution is given by the following
equation:
\multiline{
\ket{\psi(t)} &=&\frac{1}{\sqrt{\sigma \sqrt{2\pi}}} \sum_{n} e^{-\frac{(n-n_0)^2}{4\sigma^2}} \ket{n(t)} \nonumber \\
&=&\frac{1}{\sqrt{N\sigma \sqrt{2\pi}}} \sum_{n} e^{-\frac{(n-n_0)^2}{4\sigma^2}}\sum_{k} e^{-i k n} \ket{k(t)} \nonumber \\
&=& \frac{1}{N\sqrt{\sigma \sqrt{2\pi}}} \sum_{n} e^{-\frac{(n-n_0)^2}{4\sigma^2}}\sum_{k} e^{-i k n}\sum_{m} e^{i \left[k m - \frac{E(k)}{\hbar}t \right]} \ket{m(t=0)}
}

We have metioned that 
Direct optical excitation can create an exciton wavepacket which center around $k=0$ in quasi-momentum space. 
However, such a wavepacket hardly travel in real space as a whole and becomes broad very quickly because of the
shape of exciton dispersion curve. 
