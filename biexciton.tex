...
The $\hat{P}_n$ and $\hat{P}_m$ operators satisfy the bosonic commutation if $n \neq m$, and the fermionic commutation if $n = m$, as shown by \autoref{eqn:bosonCommutation} and \autoref{eqn:fermionCommutation}. 
%\begin{equation}
...
\multiline{
\text{\circled{12}} &=& \frac{1}{N_{\rm mol}} \sum_{m} G^{K}(m) e^{- i (k_1 - k_2 ) m /2} \sum_{k_1^{'} + k_2^{'} = k_1 + k_2 } \left[ E_0 + \sum_{n} J(n) e^{i k_1^{'} )n} \right] \frac{1}{\sqrt{N_{\rm mol}}} \sum_{l} C^{K}(l) e^{- i (k_1^{'} - k_2^{'} ) l /2} \nonumber \\
...
For molecules in an optical lattice, the magnitudes of $J$ and $D$ depend on the strength of the applied electric field
 and the angle between the field and the molecular array  ($\theta$), as seen in figure (\ref{fig:HowDJChanges} c).  We
...
For LiCs molecules, the condition of \autoref{biexc-formation} is satisfied for electric fields $>3.6$~kV/cm. Note that the
 ...
 in figure \ref{fig:HowDJChanges} d). Due to the linearity of the Schr\"{o}dinger equation, a positive potential is
...
where $K = k_1+k_2$ and $k = (k_1 - k_2)/2$, and $k_1$ and $k_2$ denote the wavevectors of the interacting
 ...
 Figure (\ref{fig:biexcitonSpectrum} c) shows that for $\theta = 90^o > \theta^*=\arccos(1/\sqrt{3})$ the biexciton energy is
...
figure (\ref{fig:biexcitonSpectrum} d). \Autoref{fig:HowDJChanges} and \autoref{fig:biexcitonSpectrum} thus illustrate
...
The possibility of forming Frenkel biexcitons has been proposed quite a long time ago \cite{efremov1973}. However, in 
