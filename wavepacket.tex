...
$\lambda = 1064$ nm Gaussian laser beam, with the propagation direction along the array axis, focused to a radius of
...
the direction perpendicular to the lattice plane. Assuming that
the center of the wave packet is at the beam center and the unit
length is the lattice constant $a$, the molecules in the lattice
can be indexed by the $x$, $y$ coordinates $(n_x, n_y)$. 

If the
dimension of the molecular array is smaller than one third of the
beam waist, we can approximate the gaussian intensity profile in
Eq. (\ref{gaussian-intensity-profile}) by
\begin{equation}
I(r, z=0) \approx I_{0}\left[1- \frac{2(n_x^2 +
n_y^2)a^2}{w_0^2}\right] \ ,  \label{quadratic-profile}
\end{equation}
with an accuracy of 90\%. This is a concave quadratic intensity
profile which can be used to focus a wave packet with attractive
coupling ( see the discussion in Section 3). From Figure
\ref{control-exciton}a, we can see that when the DC field is on
the lattice plane ($\theta = 0$), the coupling $\alpha$ between
molecules is negative.  This indicates that we have to apply the
DC field along the $x$-axis of the lattice.  From previous
discussions, we know that good focusing occur when $\Phi_0 =
-a/2\sigma_x$ if $\alpha<0$. In the following, we will show that a
quadratic phase profile with $\Phi_0 = -a/2\sigma_x$ can be
implemented if a relatively long laser pulse is used.
...
long coherent time ($>$ 1s) for a rotational excitation (or
exciton), the long focusing time should not be a problem.




The above analysis shows that both the completely delocalized exciton state and a partially localized Gaussian wavepacket can be focused by a quadratic phase transformation to $\sim
5$ lattice sites.  For a plane wave, the optimal kick strength
$\Phi_0^*$ explicitly depends on the array size $N$, while for a
Gaussian the initial size $\sigma_x(t=0)$ determines the phase. At
the time of focusing $t_*$, the excitation phase of a Gaussian
wave packet is site-independent, while for a plane wave the phase
in Eq.(\ref{x-focusing-PlaneWave}) varies from site to site. These
conclusions are confirmed by numerical calculations, see
Fig.~\ref{figure2}.
