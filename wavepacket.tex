...
As a 1D molecular array can be treated as a limiting case of a 2D molecular array with only a single intermolecular
...
molecules in the intermolecular axis that are at angle $\gamma$ with the $X$-axis, $\cos^2\theta_{R}$ in
 \autoref{eqn:ddM=0} can be calculated as
\multiline{
\cos^2\theta_{R} &=& \left[\cos\left(\frac{\pi}{2}-\theta\right) \cos(\phi - \gamma)\right]^2 \nonumber \\
&=& \sin^2\theta \left(\cos\phi \cos\gamma + \sin\phi \sin\gamma \right)^2 \ .
}
Then the dipole-dipole interaction between two molecules can be calculated from \autoref{eqn:ddM=0} and
