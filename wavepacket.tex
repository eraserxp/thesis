...
separation of lattice sites, the possibility of tuning the tunneling amplitude of particles  between lattice sites by
...
Such states can be produced with either bosonic or fermionic particles \cite{Greiner2002, Jordens2008}. Here, we consider
 an optical lattice fully or partially filled with one particle per lattice site, and assume that tunneling between lattice
...
wave packet to the region of the Brillouin zone near $k=\pi/2$ where the slope of the dispersion curve is steep. Similarly, to decelerate a
...
\end{eqnarray}
Equation \ref{eqn:cosineFocusing} shows that  the focusing of a wave packet onto a
 ...
 beyond the manipulation tools considered here. Creating such a wave packet may require multiple and 
...
\multiline{
C_n(t) &=& \frac{1}{\sqrt{N} }\sum_{k} e^{ i k a n} e^{-i E(k) t/\hbar} C_k(0) \nonumber \\
...
Equation \ref{eqn:widthAtFocus} shows that the wave packet will become more focused at site $n_0$ at $T_{\rm F}$ since $\sigma_{x, {\rm F}} < \sigma _{x, 0} $ and that a larger phase $\Phi_0$ will leads to a better focusing effect. However, as we 
...
$k$ components and thus adds a trivial phase to the whole wavefunction, we can safely ignore it if only the shape of
...
system with all (first neighbor, second neighbor, etc.) couplings explicitly included  in the
...
E_{e-g}$ as in  \autoref{momentum-kick}. The results are computed
...
presented in \autoref{focusing-1d} (c) and \autoref{focusing-1d} (d) for
...
focusing efficiency in 2D is greater. The results also demonstrate that the delocalized excitations can be effectively focused on different parts of the lattice simply by varying the reference site $(n_{x_0}, n_{y_0})$ in the phase transformation. 
...
\autoref{momentum-kick} and the quadratic phase transformation
at $t=0$.
 }\label{focusing-2d}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsection{Suppressing spontaneous emission in system of ultracold atoms}
...
where $k$ and $k_{\rm ex}$ represent the magnitudes of the wave vectors of the photon and the exciton, respectively. As written in
...
$z_R=73.8\; \mu$m was performed. The momentum kick of the wavepacket in $k$ space, as shown in 
\autoref{momentum-kick},  deviates only by about $7\%$ from the
...
\end{equation}
with an accuracy of 90\%. Equation \ref{quadratic-profile-1D} presents a concave quadratic intensity
...
with nearest neighbor couplings only, higher-order couplings in the case of $\alpha(n-m) \propto 1/ |n-m|^3$ 
...
visible in panels a and c of  \autoref{focusing-1d}. These oscillations are much less pronounced when all but nearest
...
by tuning the electric field direction. The thin dotted line depicts
...
The results presented in  \autoref{control-exciton} are for a 1D array of LiCs molecules in a lattice with  $a=400$ nm.
...
lattice, as shown in \autoref{control-exciton} (b).
...
presented in  \autoref{control-2d-wavepacket} show that the energy flow in two dimensions can be  
...
derivative $\dot{\Psi}$ of the wavefunction with respect to time, and the desired accuracy for the calculation,
...
zone at the next time point and the excitation population at those sites will also be lost. However, if the computation 
...
in the calculations presented in  \autoref{focusing-2d} (b). For each vacancy concentration, we carried out 48
...
with the same parameters as in  \autoref{focusing-2d}. The
...
Our calculations indicate that the focusing time may be somewhat modified by the disorder potential, even if the
 ...
 at the time of the maximal enhancement on the target molecule, chosen as molecule (71,71).   \Autoref{focusing-with-vacancy} shows that
...
\Autoref{t-matrix-focusing} shows that this choice of phases leads to effective focusing of the collective excitation in a strongly disordered system. 
...
%$\eta$ has the same definition as in  Eq. (\ref{eta}) except the time $t_*$ is arbitrarily chosen to be 4 ms here. 
...
To illustrate the efficiency of the focusing method described above, we have carried out a series of calculations 
...
distribution of vacancy sites as described above. The results are shown in  \autoref{enhancement-vs-vacancy-t-matrix}. 
