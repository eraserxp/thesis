...
&=&  \sum_{m^{\prime}}  \ket{j, m} D^{j}_{m^{\prime}, m}(R) \ . \label{eqn:JMTransformRotation}
}
...
Finally, let's consider two important properties of the rotation matrices. From the last paragraph, we know  the basis ket 
...
where $R^{-1}$ denotes the reverse of the rotation $R$. The other important property that we cover here is that the 
