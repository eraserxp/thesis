...
&=& \frac{A}{\sqrt{N}} \sum_{k} e^{-\frac{(k-k_0)^2}{2\sigma^2_{k}}} \sum_{n} e^{i \left[k n - \frac{E(k)}{\hbar}t \right]} \ket{n(t=0)} \ ,
}
where $A$ is the normalization constant. 
Expanding $E(k)$ around the point $k=k_0$ and ignore second and higher-order terms of $(k-k_0)$, and replacing the
...
determines how fast the center of the wavepacket moves in real space. 

\subsection{Phase kicking in $k$ space}

\Autoref{fig:moveWavepacket} presents an exciton dispersion. It can seen  that different regions of the 
dispersion curve correspond to different group velocities. 
Direct optical excitation can create an exciton wavepacket which center around $k=0$ in the $k$ space. 
However, such a wavepacket hardly travel in real space as a whole as their group velocity is zero. Based on 
...
\oneline{
\ket{k(t)} = \frac{1}{\sqrt{N}}\sum_n e^{i k n} \ket{n(t)} \ . \label{eqn:kStateExpansion}
}
The exponent $e^{i k n}$ in \autoref{eqn:kStateExpansion} is fixed, we can't do anything about it. 
It can be seen from \autoref{eqn:kStateExpansion} that we can change $\ket{k}$ to $\ket{k+\delta}$ by adding a
 factor $e^{i \delta n}$ to each term in the expansion.  Since the increment $\delta$ is independent of the $\ket{k}$
state, 
