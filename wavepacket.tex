...
The Gaussian intensity profile (\ref{gaussian-intensity-profile}) can also be used  to implement the quadratic phase transformations needed for focusing of collective excitations. 
To achieve this, a molecular array (1D or 2D) must be arranged in the $z=0$ plane,
...
coupling ($\alpha < 0$) ( see \autoref{sec:wavepacketFocusing}). From Figure
\ref{control-exciton} (a), we can see that when the DC field is parallel to $x$ axis of
the lattice plane ($\theta = 0$), the coupling $\alpha$ between
molecules is negative.  Since 

This indicates that we have to apply the
