...
\chapter{Green's function of two particles on a lattice}
\label{ch:greenfunc}
...
Since all the Green's functions in \autoref{eqn:eqnOfMotion} have the same parameters $n^{\prime}$ and
 $m^{\prime}$, we define 
\oneline{
...
\subsection{With nearest neighbor approximation}
\label{sec:nna}

The equation of motion for Green's function, \autoref{eqn:eqnOfMotionSimplified}, is our starting point to calculate
...
was done for a finite ordered 1D crystal of 101 lattice sites with the nearest-neighbor approximation. We set the 
energies of the particles $e_n $ to be zero,  and the dynamic interaction and the hopping interaction to be $5$. The upper panel
...
clearly demonstrate that the recursive method produces the same results as the brute-force method. 
}
...
was done for a finite disordered 1D crystal of 101 lattice sites with the nearest-neighbor approximation. We set the 
dynamic interaction and the hopping interaction to be $5$, and allow the energies of the particles $e_n$ to fluctuate
...
We are interested in the scattering of a biexciton state through disorders. For convenience, the disorders are assumed
...
Substituting \autoref{eqn:Gexpansion} into \autoref{eqn:scatteringStateInSeries}, we obtain
\oneline{
...
In this chapter, a recursive method to calculate lattice Green's functions is described. This essential idea 
